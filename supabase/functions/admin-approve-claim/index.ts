...
serve(async (req) => {
  ...
  try {
    ...
    // 9. Execute approve_claim RPC
    const { data, error } = await supabaseAdmin.rpc('approve_claim', { claim_id })

    // 10. If RPC fails
    if (error) {
      console.error('approve_claim RPC error:', error)
      return new Response(JSON.stringify({ error: error.message || 'Error executing claim approval' }), {
